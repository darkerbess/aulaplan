...
import { createDate, cloneDate, addDuration, createDuration, DAY_IN_SECONDS, sortEventChunks, setContent, hasYScroll, maybeIgnore, toEventWithLocalDates, toViewWithLocalDates, createEventContent, createEventChunk, datesEqual, toLocalDate, toISOString, setFn, rect, height, setMidnight, addDay, prepareEventChunks } from '@event-calendar/common';
...
// (120:8) {#each bgChunks as chunk (chunk.event)}
function create_each_block_1$1(key_1, ctx) {
	...
	return {
		...
		p(new_ctx, dirty) {
			...
			if (dirty[0] & /*bgChunks*/ 8) event_changes.chunk = /*chunk*/ ctx[43];
...
// (129:8) {#each chunks as chunk (chunk.event)}
function create_each_block$3(key_1, ctx) {
	...
	return {
		...
		p(new_ctx, dirty) {
			...
			const event_changes = {};
			if (dirty[0] & /*date*/ 1) event_changes.date = /*date*/ ctx[0];
			if (dirty[0] & /*chunks*/ 4) event_changes.chunk = /*chunk*/ ctx[43];
			event.$set(event_changes);
		},
		i(local) {
			if (current) return;
...
// (133:8) {#if iChunks[0] && !iChunks[0].event.allDay}
...
function create_fragment$4(ctx) {
	...
	const get_key_1 = ctx => /*chunk*/ ctx[43].event;
...
function instance$4($$self, $$props, $$invalidate) {
	...
	$$self.$$.update = () => {
		...
		if ($$self.$$.dirty[0] & /*start, end, bgChunks, chunks*/ 1610612748 | $$self.$$.dirty[1] & /*$_events*/ 16) {
			{
				$$invalidate(2, chunks = []);
...
// (80:4) {#if iChunks[0] && datesEqual(iChunks[0].date, date)}
...
function create_each_block$2(ctx) {
	...
	return {
		...
		p(ctx, dirty) {
			const event_changes = {};
			if (dirty[0] & /*dayChunks*/ 16) event_changes.chunk = /*chunk*/ ctx[29];
...
function instance$2($$self, $$props, $$invalidate) {
	...
	$$self.$$.update = () => {
		if ($$self.$$.dirty[0] & /*chunks, date, dayChunks*/ 1048593) {
			{
